			...
			try
			{
				...
				if (viewModel != null && viewModel != null && IsVendor)
				{
					if (!Request.Form.ContainsKey("ProjectMap"))
					{
						...
						if (!string.IsNullOrEmpty(viewModel.ContactNo) && !System.Text.RegularExpressions.Regex.IsMatch(viewModel.ContactNo, @"^\d{10}$"))
                        {
                            CommonViewModel.IsSuccess = false;
                            CommonViewModel.StatusCode = ResponseStatusCode.Error;
                            CommonViewModel.Message = "Contact No must be exactly 10 digits.";
